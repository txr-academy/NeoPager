...
#define DEVICE_ID  1   // set to 2 on second board
...
const unsigned long ACK_TIMEOUT_MS = 6000;   // wait 3s before retrying
...
void scrollBigMessage(const char* header, const char* message, int repeatCount, const char* footer = "") {
  ...
  for (int rep = 0; rep < repeatCount; rep++) {
    if (textWidth <= screenW) {
      ...
      u8g2.drawFrame(0, 0, 128, 64);
      u8g2.sendBuffer();
      // hold but keep checking for ACK
      unsigned long t = millis();
      while (millis() - t < 2000) {
        if (radioFlag) return;   // ACK arrived — exit scroll immediately
        delay(20);
      }
    } else {
...
void handleIncoming() {
  ...
  if (state == RADIOLIB_ERR_NONE) {
    ...
    if (type == "MSG") {
      ...
      // Board 1 waits 3s for ACK — we must reply within ~500ms
      sendAck(sender, msgId);
      delay(50);           // tiny gap so ACK clears the antenna
      startListening();    // back to RX immediately after ACK

      // now safe to do slow UI stuff
